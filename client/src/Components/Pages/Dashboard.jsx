...
const Dashboard = () => {
  ...
  const { data: budgets = [], isLoading: budgetLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://toolkit-backend-c3ua.onrender.com/budget/${user?.email}`
      );
      return res.json();
    },
  });
        ...
        <div className="p-6 md:p-10 w-full max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-4 md:py-6">
            ...
            >
              ...
              {budgetLoading ? (
                <p className="text-gray-400 mt-2">Loading budget...</p>
              ) : (
                <p
            ...
            >
              ...
              {tasksLoading ? (
                ...
                <p className="text-gray-600 mt-2">
                  ...
                  {remainingTasks.length !== 1 && "s"} left to complete
                </p>
              )}
            ...
            {/* Quiz Card (doesn’t need loading) */}
            <Link
              to="/quiz"
              className="bg-white shadow-md rounded-2xl p-5 hover:shadow-lg transition flex flex-col justify-between"
            >
